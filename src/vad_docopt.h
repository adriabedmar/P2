...
typedef struct {
    ...
    char *input_wav;
    char *noise_hysteresis;
    char *noise_threshold;
    char *output_vad;
    char *output_wav;
    char *silence_voiced;
    char *voiced_silence;
...
"\n"
"Usage:\n"
"   vad [options] -i <input-wav> -o <output-vad> [-w <output-wav>] [--noise-threshold=<k>] [--noise-hysteresis=<l>] [--silence-voiced=<m>] [--voiced-silence=<n>]\n"
"   vad (-h | --help)\n"
"   vad --version\n"
"\n"
"Options:\n"
...
"   -k char, --noise-threshold=<k>     Threshold over noise to determine the sound as voiced [default: 5]\n"
"   -l char, --noise-hysteresis=<l>    Threshold over a1 that the sound has to surpass in a certain time to determine the sound as voiced [default: 4]\n"
"   -m char, --silence-voiced=<m>      Maximum time (frames) to surpass the threshold and determine that the section has passed from silence to voice [default: 2]\n"
"   -n char, --voiced-silence=<n>      Maximum time (frames) to surpass the threshold and determine that the section has passed from voice to silence [default: 11]\n"
...
int elems_to_args(Elements *elements, DocoptArgs *args, bool help,
    ...
    for (i=0; i < elements->n_options; i++) {
        ...
        } else if (!strcmp(option->olong, "--noise-hysteresis")) {
            if (option->argument)
                args->noise_hysteresis = option->argument;
        } else if (!strcmp(option->olong, "--noise-threshold")) {
            if (option->argument)
                args->noise_threshold = option->argument;
        ...
        } else if (!strcmp(option->olong, "--silence-voiced")) {
            if (option->argument)
                args->silence_voiced = option->argument;
        } else if (!strcmp(option->olong, "--voiced-silence")) {
            if (option->argument)
                args->voiced_silence = option->argument;
        }
    }
    /* commands */
...
DocoptArgs docopt(int argc, char *argv[], bool help, const char *version) {
    DocoptArgs args = {
        0, 0, 0, NULL, (char*) "4", (char*) "5", NULL, NULL, (char*) "2",
        (char*) "11",
    ...
    Option options[] = {
        ...
        {"-l", "--noise-hysteresis", 1, 0, NULL},
        {"-k", "--noise-threshold", 1, 0, NULL},
        {"-o", "--output-vad", 1, 0, NULL},
        {"-w", "--output-wav", 1, 0, NULL},
        {"-m", "--silence-voiced", 1, 0, NULL},
        {"-n", "--voiced-silence", 1, 0, NULL}
    };
